...
fn main() {
    ...
    if Path::new("config.ini").exists() {
        config
            .read(fs::read_to_string("config.ini").unwrap())
            .unwrap();

        color = if config.get("terminal", "color").is_some() {
            if config
                .get("terminal", "color")
                .unwrap()
                .parse::<i8>()
        ...
        ignore_warnings = if config.get("terminal", "ignore-warnings").is_some() {
            if config
                .get("terminal", "ignore-warnings")
                .unwrap()
                .parse::<bool>()
                .is_err()
            {
                ...
                config.get("terminal", "ignore-warnings").unwrap() == "true"
            }
        } else {
            false
        };

        ls = if config.get("terminal", "ls").is_some() {
        ...
        font = if config.get("terminal", "font").is_some() {
        ...
        clear_type = if config.get("terminal", "clear").is_some() {
    ...
    let mut debug = if config.get("app", "debug").is_some() {
        if config.get("app", "debug").unwrap().parse::<bool>().is_err() {
    ...
    loop {
        ...
        if Path::new("config.ini").exists() {
            ...
            } else if config.get("terminal", "prompt").unwrap() == "default" {
            ...
            match (first_arg, second_arg) {
                ...
                ("cd", arg) => {
                    if Path::new(arg).exists() {
                        if let Err(e) = set_current_dir(arg) {
                            ...
                            println!(
                                "\x1b[1;36mCurrent Directory changed to \x1b[1;32m{}\x1b[0m",
                ...
                ("debug", second_arg) if !second_arg.is_empty() => {
                    if second_arg.trim() == "on" || second_arg.trim() == "true" {
                        ...
                        println!(
                            "\x1b[1;32mSuccesfully: \x1b[0;32mChanged debug to {}",
                            second_arg
                        );
                ...
                ("color", second_arg) if !second_arg.is_empty() => {
                    ...
                    } else if second_arg.parse::<i8>().is_err() {
                        println!(
                            "\x1b[1;31mError: \x1b[0;31mSecond Argument should be a digit\x1b[0m"
                        );
                    }
                ...
                ("editcf" | "edit", second_arg) if second_arg.contains('=') => {
                    ...
                    if option == "color=" {
                        let backup_color = config
                            .get("terminal", "color")
                            .unwrap()
                            .parse::<i8>()
                            .unwrap();

                        color = if value.parse::<i8>().is_ok() {
                            value.parse().unwrap()
                        } else {
                    ...
                    config.set(
                        "terminal",
                        &option.replace('=', ""),
                        Some(second_arg.replace(&option, "")),
                    );
                    config.write("config.ini").unwrap();
                    set_current_dir(current).unwrap();
                }
                ("check" | "check_path", second_arg) => {
                    if Path::new(second_arg.trim()).exists() {
                        ...
                        println!(
                            "\x1b[1;31mPath: \x1b[0;31m'{}' does not exist\x1b[0m\n",
                            second_arg
                        );
                    }
                }
        ...
        match input.trim() {
                ...
                loop {
                    ...
                    if input.trim() != "exit" && input.trim() != "close" {
                        ...
                        if let Err(e) = interpreter.evaluate(input.trim()) {
                            ...
                            println!(
                                "\x1b[1;32mResult: \x1b[0;32m{}\x1b[0m\n",
            ...
            "ignore-warnings" | "ignore-warning" => {
                ...
                config.set(
                    "terminal",
                    "ignore-warnings",
                    Some(ignore_warnings.to_string()),
                );
                config.write("config.ini").unwrap();
            }
            "pwd" => println!("\x1b[0m{}\n", current_dir().unwrap().display()),
            "cls" | "clear" => match &*clear_type {
                "default" => {
                    if debug {
            ...
            "color" => {
                println!("\x1b[0;32mType color ? to view a list of colors\n\x1b[0;35mE.g color 1\n")
            }
            "f" | "forth" => {
                println!("\x1b[1;36mf:");
            ...
            "dir" | "ls" => {
                ...
                if debug {
                    println!("\x1b[1;32mDirectories: {}", dirs.len());
            ...
            _ => {
                if input.trim().contains("print") {
                        ...
                        match (i, str) {
                            ...
                            (char, 1) => {
                                if char != '"' && char != '\'' {
                                    ...
                                    tokens.insert(
                                        count,
                                        Type::String(string.clone().replace('\"', "")),
                                    );
                                    str += 1;
                                }
                            }
                    ...
                    for i in input.trim().chars() {
                        match i {
                            ...
                            alpha if alpha.is_numeric() => {
                                tokens.push(Type::Number(i.to_string().parse::<f64>().unwrap()))
                            }
                            _ => (),
                        }
                    }
                    ...
                    let mut input_list = input.split_whitespace();
                    ...
                    if first_arg.is_none() {
                        ...
                        continue;
                    } else if Path::new(home_directory).exists() {
                        set_current_dir(home_directory).unwrap();
                        if cfg!(target_os = "windows") {
                            if let Err(e) = Command::new(first_arg.unwrap())
                                ...
                                .status()
                            {
                                ...
                                println!(
                                    "\x1b[0;32m`{}` succesfully ran\x1b[0m\n",
                                    first_arg.unwrap()
                                );
                            }
                        } else if cfg!(target_os = "linux")
                            || cfg!(target_os = "macos") && Path::new(first_arg.unwrap()).exists()
                        {
                            if let Err(e) = Command::new(first_arg.unwrap())
                                .current_dir(home_directory)
                                .status()
                        ...
                        println!(
                            "\x1b[1;31mError: \x1b[0;31mFailed to launch `{}`: Doesnt exist\n",
                            first_arg.unwrap()
                        );
                        continue;
                    }
